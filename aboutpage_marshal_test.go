...
func TestAboutPage_marshal(t *testing.T) {
	...
	type EmbeddedAboutPage struct {
		AboutPage schemaorg.AboutPage `json:"aboutPage,omitempty"`
	}
	...
	tests := []struct{
		...
		{
			Value: schemaorg.AboutPage{},
			Expected: []byte(`{`+context+`,`+`"@type":"AboutPage"}`),
		},
		{
			Value: EmbeddedAboutPage{},
		...
		{
			Value: EmbeddedAboutPage{
				AboutPage: schemaorg.AboutPage{

					Name:        opt.Something("About Us"),
					Description: opt.Something("Learn more about our company and mission."),
					URL:         opt.Something("https://example.com/about"),

					Author:        opt.Something("Example Inc."),
					DatePublished: opt.Something("2024-01-15"),

					LastReviewed: opt.Something("2024-06-01"),
				},
			},
			Expected: []byte(
				`{`+
