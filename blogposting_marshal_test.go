...
func TestBlogPosting_marshal(t *testing.T) {
	...
	type EmbeddedBlogPosting struct {
		BlogPosting schemaorg.BlogPosting `json:"blogPosting,omitempty"`
	}
	...
	tests := []struct{
		...
		{
			Value: schemaorg.BlogPosting{},
			Expected: []byte(`{`+context+`,"@type":"BlogPosting"}`),
		},
		{
			Value: EmbeddedBlogPosting{},
...

		{
			Value: EmbeddedBlogPosting{
				BlogPosting: schemaorg.BlogPosting{
					...
					Name: opt.Something("My First Blog Post"),
					URL:  opt.Something("https://example.com/blog/first-post"),

					Author:        opt.Something("Jane Blogger"),
					DatePublished: opt.Something("2024-03-20"),
					Headline:      opt.Something("Getting Started with Blogging"),

					ArticleBody:    opt.Something("This is the body of my first blog post..."),
					ArticleSection: opt.Something("Technology"),

					SharedContent: opt.Something("https://example.com/shared-image.jpg"),
				},
			},
			Expected: []byte(
				`{`+
					...
					`"blogPosting":{`+
						...
						`,`+
						`"headline":"Getting Started with Blogging"`+
						`,`+
						`"articleBody":"This is the body of my first blog post..."`+
						`,`+
						`"articleSection":"Technology"`+
						`,`+
						`"sharedContent":"https://example.com/shared-image.jpg"`+
					`}`+
				`}`),
		},
	}
