...
func TestFAQPage_marshal(t *testing.T) {
	...
	type EmbeddedFAQPage struct {
		FAQPage schemaorg.FAQPage `json:"faqpage,omitempty"`
	}
	...
	tests := []struct{
	...
	}{
		{
			...
			Expected: []byte(`{`+context+`,"@type":"FAQPage"}`),
		},
		{
			Value: EmbeddedFAQPage{},
			...
			Expected: []byte(
				`{`+
					...
					`"faqpage":{`+
						...
						`,`+
						`"mainEntity":[`+
							`{"@type":"Question","name":"What is Go?","acceptedAnswer":{"@type":"Answer","text":"Go is a programming language."}}`+
							`,`+
							`{"@type":"Question","name":"Is Go fast?","acceptedAnswer":{"@type":"Answer","text":"Yes, Go is very fast."}}`+
						`]`+
					`}`+
				`}`),
		},
