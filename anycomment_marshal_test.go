...
func TestAnyComment_marshal(t *testing.T) {
	...
	type EmbeddedAnyComment struct {
		AnyComment schemaorg.AnyComment `json:"anyComment,omitempty"`
	}
	...
	tests := []struct{
		...
		{
			Value: schemaorg.AnyComment{},
			Expected: []byte(`{`+context+`}`),
		},
		{
			Value: EmbeddedAnyComment{},
			Expected: []byte(`{`+context+`}`),
		},
		...
		{
			Value: EmbeddedAnyComment{
				AnyComment: schemaorg.AnyComment{

					Name:   opt.Something("A Comment"),
					URL:    opt.Something("https://example.com/comment/1"),

					Author:        opt.Something("Jane Doe"),
					DatePublished: opt.Something("2024-07-10"),
					Text:          opt.Something("This is a great article!"),

					DownvoteCount: opt.Something("2"),
					UpvoteCount:   opt.Something("15"),
				},
			},
			Expected: []byte(
				`{`+
