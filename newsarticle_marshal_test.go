...
func TestNewsArticle_marshal(t *testing.T) {
	...
	tests := []struct{
		...
		{
			Value: schemaorg.NewsArticle{},
			Expected: []byte(`{`+context+`,"@type":"NewsArticle"}`),
		},
		{
			Value: EmbeddedNewsArticle{},
		...
		{
			Value: EmbeddedNewsArticle{
				NewsArticle: schemaorg.NewsArticle{

					Name: opt.Something("City Council Approves New Park"),
					URL:  opt.Something("https://example.com/news/city-council-park"),

					Author:        opt.Something("John Reporter"),
					DatePublished: opt.Something("2024-08-20"),
					Headline:      opt.Something("City Council Approves New Park Development"),

					ArticleBody:    opt.Something("The city council voted unanimously to approve..."),
					ArticleSection: opt.Something("Local News"),

					Dateline:     opt.Something("NEW YORK, Aug. 20"),
					PrintColumn:  opt.Something("3"),
					PrintEdition: opt.Something("Morning Edition"),
					PrintSection: opt.Something("A"),
				},
			},
			Expected: []byte(
				`{`+
					...
					`"newsArticle":{`+
						`"@type":"NewsArticle"`+
						`,`+
						`"name":"City Council Approves New Park"`+
						`,`+
						`"url":"https://example.com/news/city-council-park"`+
						`,`+
						...
						`,`+
						`"articleBody":"The city council voted unanimously to approve..."`+
						`,`+
