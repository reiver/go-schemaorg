...
func TestSearchResultsPage_marshal(t *testing.T) {
	...
	type EmbeddedSearchResultsPage struct {
		SearchResultsPage schemaorg.SearchResultsPage `json:"searchResultsPage,omitempty"`
	}
	...
	tests := []struct{
		...
		{
			Value: schemaorg.SearchResultsPage{},
			Expected: []byte(`{`+context+`,`+`"@type":"SearchResultsPage"}`),
		},
		{
			Value: EmbeddedSearchResultsPage{},
		...
		{
			Value: EmbeddedSearchResultsPage{
				SearchResultsPage: schemaorg.SearchResultsPage{

					Name:        opt.Something("Search Results"),
					Description: opt.Something("Results for your query."),
					URL:         opt.Something("https://example.com/search?q=test"),

					DatePublished: opt.Something("2024-03-10"),

					Breadcrumb: opt.Something("Home / Search"),
				},
			},
			Expected: []byte(
				`{`+
					...
					`"searchResultsPage":{`+
						...
						`,`+
						`"description":"Results for your query."`+
						`,`+
						`"name":"Search Results"`+
						`,`+
