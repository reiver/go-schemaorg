...
func TestDataset_marshal(t *testing.T) {
	...
	tests := []struct{
		...
		{
			Value: schemaorg.Dataset{},
			Expected: []byte(`{`+context+`,"@type":"Dataset"}`),
		},
		{
			Value: EmbeddedDataset{},
		...
		{
			Value: EmbeddedDataset{
				Dataset: schemaorg.Dataset{

					Name:        opt.Something("World Population Data"),
					Description: opt.Something("A dataset containing world population statistics."),
					URL:         opt.Something("https://example.com/datasets/world-population"),
				},
			},
			Expected: []byte(
				`{`+
					...
					`"dataset":{`+
						`"@type":"Dataset"`+
						`,`+
						`"description":"A dataset containing world population statistics."`+
						`,`+
						`"name":"World Population Data"`+
						`,`+
						`"url":"https://example.com/datasets/world-population"`+
					`}`+
				`}`),
		},
...

		{
			Value: EmbeddedDataset{
				Dataset: schemaorg.Dataset{
					...
					Name:                  opt.Something("Climate Measurements"),
					Description:           opt.Something("Climate measurement data from weather stations."),
					License:               opt.Something("https://creativecommons.org/licenses/by/4.0/"),
					Distribution:          opt.Something("https://example.com/datasets/climate/download"),
					IncludedInDataCatalog: opt.Something("https://example.com/catalog"),
					ISSN:                  opt.Something("1234-5678"),
					MeasurementTechnique:  opt.Something("Thermometer readings"),
					VariableMeasured:      opt.Something("Temperature"),
				},
			},
			Expected: []byte(
				`{`+
					...
					`"dataset":{`+
						`"@type":"Dataset"`+
						`,`+
						`"description":"Climate measurement data from weather stations."`+
						`,`+
						`"name":"Climate Measurements"`+
						`,`+
						`"license":"https://creativecommons.org/licenses/by/4.0/"`+
						`,`+
						`"distribution":"https://example.com/datasets/climate/download"`+
						`,`+
						`"includedInDataCatalog":"https://example.com/catalog"`+
						`,`+
						`"issn":"1234-5678"`+
						`,`+
						`"measurementTechnique":"Thermometer readings"`+
						`,`+
