...
func TestDataFeed_marshal(t *testing.T) {
	...
	tests := []struct{
		...
		{
			Value: schemaorg.DataFeed{},
			Expected: []byte(`{`+context+`,"@type":"DataFeed"}`),
		},
		{
			Value: EmbeddedDataFeed{},
...

		{
			Value: EmbeddedDataFeed{
				DataFeed: schemaorg.DataFeed{
					...
					Name:        opt.Something("Product Price Feed"),
					Description: opt.Something("A real-time feed of product prices."),
					URL:         opt.Something("https://example.com/feeds/prices"),
				},
			},
			Expected: []byte(
				`{`+
					...
					`"dataFeed":{`+
						`"@type":"DataFeed"`+
						`,`+
						`"description":"A real-time feed of product prices."`+
						`,`+
						`"name":"Product Price Feed"`+
						`,`+
						`"url":"https://example.com/feeds/prices"`+
					`}`+
				`}`),
		},
...

		{
			Value: EmbeddedDataFeed{
				DataFeed: schemaorg.DataFeed{
					...
					Name:            opt.Something("Stock Ticker Feed"),
					Description:     opt.Something("Live stock price data feed."),
					DataFeedElement: opt.Something("https://example.com/feeds/stocks/item/1"),
					Distribution:    opt.Something("https://example.com/feeds/stocks/download"),
				},
			},
			Expected: []byte(
				`{`+
					...
					`"dataFeed":{`+
						`"@type":"DataFeed"`+
						`,`+
						`"description":"Live stock price data feed."`+
						`,`+
						`"name":"Stock Ticker Feed"`+
						`,`+
						`"distribution":"https://example.com/feeds/stocks/download"`+
						`,`+
						`"dataFeedElement":"https://example.com/feeds/stocks/item/1"`+
					`}`+
				`}`),
		},
	}
